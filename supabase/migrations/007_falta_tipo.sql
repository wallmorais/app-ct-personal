-- PT.Control — Falta Avisada vs Não Avisada
-- 'avisada' segue o fluxo normal de falta (status='falta', não cobra, gera
-- reposição). 'nao_avisada' é tratada como aula realizada (status='presente',
-- cobra, sem reposição), mas preserva aqui o histórico de que houve ausência
-- sem aviso, para relatórios/auditoria. Não cria tabela nova.

alter table public.registros add column if not exists falta_tipo text
  check (falta_tipo is null or falta_tipo in ('avisada', 'nao_avisada'));
